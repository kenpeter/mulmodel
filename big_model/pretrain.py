...
Memory strategy — GaLore optimizer:
  Model in BF16 on GPU          → ~2.4 GB VRAM
  GaLore optimizer states       → ~1.2 GB VRAM  (rank-128 projection, stays on GPU)
  Activations (no grad ckpt)    → ~2.5 GB VRAM
  Peak VRAM:                    → ~6-8 GB  (fits in 12 GB, no CPU needed)

  GaLore projects each gradient matrix to rank-128 before Adam,
  so m and v are 8-10× smaller than full AdamW. No CPU offload needed.
...
GALORE_UPDATE    = 200   # recompute projection every N steps
GALORE_SCALE     = 0.25  # gradient scale factor
...
class BigModelPretrainer:
    ...
    Memory layout (GaLore):
      - BF16 model on GPU       ~2.4 GB
      - GaLore optimizer states ~1.2 GB  (rank-128, on GPU, no CPU needed)
      - Activations             ~2.5 GB
      - Peak VRAM               ~6-8 GB
    """
    ...
    def __init__(self, device: str | None = None, resume: bool = True) -> None:
        ...
        # ── GaLore optimizer ─────────────────────────────────────────────────
        # Large weight matrices (ndim≥2) get low-rank projected gradients.
        # Biases and layer norms get regular Adam.
        galore_params  = []
        regular_params = []
        ...
        param_groups = [
            {"params": regular_params},
            {
                ...
                "rank":            GALORE_RANK,
                "update_proj_gap": GALORE_UPDATE,
                "scale":           GALORE_SCALE,
                "proj_type":       "std",
            },
        ]
        self.optim = GaLoreAdamW(param_groups, lr=LR, weight_decay=0.01)

        n    = self.model.param_count()
        vram = torch.cuda.memory_allocated() / 1e9 if use_gpu else 0
        n_galore = sum(p.numel() for p in galore_params)
        print(f"  [BigModel] {n:,} params  BF16 on GPU={use_gpu}")
        print(f"  [BigModel] VRAM after model load: {vram:.2f} GB")
        print(f"  [BigModel] GaLore rank={GALORE_RANK} on {n_galore:,} params "
              f"(~{n_galore*2*GALORE_RANK/(n_galore)*100/100:.0f}% of optimizer states vs full AdamW)")
    ...
    def train(self, n_epochs: int = 10, steps_per_epoch: int = 50) -> BigModel:
        ...
        print(f"  [BigModel] Optimizer: GaLoreAdamW  rank={GALORE_RANK}  update_gap={GALORE_UPDATE}")
