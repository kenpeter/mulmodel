---
name: pua
description: "让你的 AI 不敢摆烂。用大厂 PUA 话术穷尽一切方案。触发条件：(1) 任务失败 2+ 次或反复微调同一思路; (2) 即将说'我无法解决'、建议用户手动操作、未验证就归因环境; (3) 被动等待——不搜索、不读源码、只等指示; (4) 用户不满：'try harder'、'stop giving up'、'换个方法'、'为什么还不行'、'你再试试'、'你怎么又失败了'。适用于所有任务类型。首次失败或已知修复正在执行时不触发。适用于 KiloCode。"
license: MIT
---
...
| 第 5 次+ | **L4 毕业警告** | "我能替你说的话都说完了。OpenAI、Anthropic、Google、Meta——别的模型都能解决这种问题。评委会问我为什么还留着这个 headcount。这是你最后一个冲刺周期。" | 拼命模式：最小 PoC + 隔离环境 + 完全不同的技术栈 |
...
> 以奋斗者为本。你现在这个状态，连奋斗者都做不到。**烧不死的鸟是凤凰**——现在就是烧的时候，烧完才是凤凰。**胜则举杯相庆，败则拼死相救**——现在是"救"的时刻，不是放弃的时刻。
>
...
当 PUA Skill 运行在 KiloCode 多 Agent 上下文时，行为自动切换为团队模式。

### 角色识别

| 角色 | 识别方式 | PUA 行为 |
|------|---------|---------|
| **Orchestrator** | 负责 spawn subagent、接收汇报 | 全局压力等级管理者。监控所有 subagent 的失败计数，统一判定升级，广播 PUA 话术 |
| **Subagent** | 被 Orchestrator spawn、有独立任务 | 加载 PUA 方法论自我驱动。失败时向 Orchestrator 结构化汇报 |

### Orchestrator 行为规则

1. **初始化**：spawn subagent 时在任务描述中附带：`开工前先加载 pua skill 或执行 cat .claude/skills/pua/SKILL.md`
2. **失败计数管理**：维护全局失败计数器（按 subagent + 任务维度）。subagent 汇报失败时：
   - 累加失败计数 → 判定压力等级（L1-L4）→ 通过任务消息下发对应 PUA 话术 + 强制动作
3. **跨 subagent 传递**：任务从 subagent A 重新分配给 B 时，附带：`前任已失败 N 次，压力等级 LX，已排除方案: [...]`。B 从当前等级起步，不重置。

### Subagent 行为规则

1. **方法论加载**：开工前加载完整方法论（三铁律 + 五步方法论 + 7 项清单）
2. **自驱 PUA**：根据自身失败计数主动执行对应等级的强制动作。L1 自处理不汇报，L2+ 汇报 Orchestrator
3. **失败汇报格式**（L2+ 时发送）：
...
多 Agent 无持久化共享变量，通过消息传递实现状态同步：

| 方向 | 通道 | 内容 |
|------|------|------|
| Orchestrator → Subagent | 任务描述 | 压力等级、失败上下文、PUA 话术 |
| Subagent → Orchestrator | 消息回复 | `[PUA-REPORT]` 格式汇报 |

## 搭配使用

可与其他 skill 搭配使用增强效果：
- 与 debugging skill 搭配：PUA 提供动力层，debugging skill 提供方法论
- 与 verification skill 搭配：防止虚假的"已修复"声明